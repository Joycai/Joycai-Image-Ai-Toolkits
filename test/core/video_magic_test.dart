...
void main() {
  List<int> ftyp(String brand) =>
      [0, 0, 0, 0x20, ...ascii.encode('ftyp'), ...ascii.encode(brand)];
  ...
  test('EBML magic is .webm', () {
    expect(videoExtensionFromBytes([0x1A, 0x45, 0xDF, 0xA3, 0x9F, 0x42]),
        '.webm');
  });

  test('an HTML error page or a JSON body is not a video', () {
    expect(videoExtensionFromBytes(utf8.encode('<!DOCTYPE html><html>')),
        isNull);
    expect(videoExtensionFromBytes(utf8.encode('{"error":"expired"}')),
        isNull);
