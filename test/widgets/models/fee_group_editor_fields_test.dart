...
void main() {
  ...
  testWidgets('the editor shows the input row under request and spec, not under token', (
    tester,
  ) async {
    final draft = FeeGroupDraft(
      PricingGroup(id: 1, name: 'Relay', billingMode: 'request', requestPrice: 0.08),
    );
  ...
  testWidgets('a request group that charges inputs gets the input tag on its card', (tester) async {
          ...
          return Column(
            children: [
              Row(
                ...
                children: feeGroupPriceTags(context, l10n, charging),
              ),
    ...
    expect(
      find.descendant(
        of: find.byKey(const ValueKey('charging')),
        matching: find.byType(FeePriceTag),
      ),
      findsNWidgets(2),
    );
