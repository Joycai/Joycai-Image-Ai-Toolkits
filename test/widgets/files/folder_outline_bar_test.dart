...
void main() {
  ...
  testWidgets('narrowing the bar walks the levels in the declared order', (tester) async {
  ...
  testWidgets('a tap jumps to that chip, and the lit chip follows the index', (tester) async {
    tester.view.physicalSize = const Size(2400, 600);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.reset);
    final current = ValueNotifier<int>(0);
    ...
    await tester.pumpWidget(
      host(width: 2000, items: entries(4), current: current, onJump: jumps.add),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'a secondary tap opens the chip menu; re-authorize replaces show-only on a locked chip',
    (tester) async {
      tester.view.physicalSize = const Size(2400, 600);
      tester.view.devicePixelRatio = 1;
      addTearDown(tester.view.reset);
