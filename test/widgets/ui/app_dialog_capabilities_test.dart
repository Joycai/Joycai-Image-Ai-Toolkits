...
void main() {
    ...
    await tester.pumpWidget(
      host(
        AppDialog(
          ...
          maxHeight: 240,
          scrollable: true,
          content: Column(children: [for (var i = 0; i < 60; i++) Text('Line $i')]),
        ),
      ),
    );
  ...
  group('heading', () {
    ...
    testWidgets('icon takes the accent unless told otherwise', (tester) async {
      await tester.pumpWidget(
        host(
          const AppDialog(title: 'Delete', icon: Icons.warning_amber, content: SizedBox.shrink()),
        ),
      );
    ...
    testWidgets('a title and a titleWidget together is a mistake, not a silent win', (
      tester,
    ) async {
      expect(
        () => AppDialog(title: 'A', titleWidget: const Text('B'), content: const SizedBox.shrink()),
