...
void main() {
  ...
  group('a rich item\'s trailing', () {
    ...
    ({Rect row, Rect label, Rect trailing}) boxes(
      WidgetTester tester,
      String label,
      String trailing,
    ) {
      ...
      final labelBox = find
          .ancestor(of: find.text(label).last, matching: find.byType(Expanded))
          .first;
      return (
        row: tester.getRect(row),
        label: tester.getRect(labelBox),
        trailing: tester.getRect(trailingText),
      );
    }

    // A 390pt phone page, a 320pt device, and narrower still.
    for (final width in [366.0, 296.0, 240.0]) {
      testWidgets('a long one is cut at its share and the name keeps the rest @ $width', (
        tester,
      ) async {
        ...
        expect(
          b.label.width,
          greaterThan(b.row.width * 0.5),
          reason: 'the name is what is being chosen',
        );
        expect(b.trailing.right, closeTo(b.row.right, 0.5));
      });
    }

    testWidgets('a short one is shown whole, flush right, and takes nothing it does not need', (
      tester,
    ) async {
      await open(tester, 422, 'Flux', short);

      final b = boxes(tester, 'Flux', short);
      final intrinsic = (tester.renderObject(find.text(short).last) as RenderBox)
      ...
      expect(
        b.label.right,
        closeTo(b.trailing.left - 12, 0.5),
        reason: 'the label box has all that is left',
      );
    });
    ...
    testWidgets('the closed field never builds the menu row, whichever item is selected', (
      tester,
    ) async {
      ...
      expect(
        find.descendant(of: find.byType(AppDropdown<int>), matching: find.byType(LayoutBuilder)),
        findsNothing,
      );
    });
