...
void main() {
  ...
  Future<void> open(
    WidgetTester tester,
    List<AppGlassMenuEntry> entries, {
  ...
  testWidgets('the chosen action runs once the menu is no longer the current route', (
    tester,
  ) async {
    ...
    expect(
      hostWasCurrent,
      isTrue,
      reason: 'the action ran while the menu was still the current route',
    );
  ...
  testWidgets('a disabled row does nothing and can say why', (tester) async {
    ...
    await open(tester, [
      AppGlassMenuItem(
        label: 'Move to…',
        note: 'Root folders cannot be moved',
        enabled: false,
        onSelected: () => ran = true,
      ),
    ...
    expect(ran, isFalse);
    expect(
      find.byType(AppGlassMenu),
      findsOneWidget,
      reason: 'tapping a disabled row closed the menu',
    );
  });
  ...
  testWidgets('a menu opened near the corner flips to the other side of the click', (tester) async {
    ...
    expect(
      rect.right,
      lessThanOrEqualTo(click.dx + 0.01),
      reason: 'menu $rect ran past the click horizontally',
    );
    expect(
      rect.bottom,
      lessThanOrEqualTo(click.dy + 0.01),
      reason: 'menu $rect ran past the click vertically',
    );
  ...
  testWidgets(
    'a submenu opens beside the menu, level with its row, and its action runs after the pop',
    (tester) async {
      await pumpHost(tester);
      var ran = false;
      ...
      expect(
        sub.left,
        closeTo(menu.right + 4, 0.01),
        reason: 'submenu $sub is not 4px right of the menu $menu',
      );
  ...
  testWidgets(
    'the submenu follows the pointer: opens on its row, survives the crossing, closes on another row',
    (tester) async {
      await pumpHost(tester);
      ...
      expect(
        find.byType(AppGlassMenu),
        findsNWidgets(2),
        reason: 'hovering the row did not open its submenu',
      );
      ...
      expect(
        find.byType(AppGlassMenu),
        findsNWidgets(2),
        reason: 'crossing into the submenu closed it',
      );
      ...
      expect(
        find.byType(AppGlassMenu),
        findsOneWidget,
        reason: 'hovering another row left the submenu open',
      );
    },
  );
  ...
  testWidgets('→ opens a submenu from the keyboard and ← closes it again', (tester) async {
    ...
    await open(tester, [
      AppGlassMenuItem(
        label: 'File',
        children: [AppGlassMenuItem(label: 'Rename', onSelected: () {})],
      ),
    ]);
  ...
  testWidgets('a submenu that would run off the right edge opens on the left instead', (
    tester,
  ) async {
    await pumpHost(tester);
    ...
    expect(
      sub.right,
      closeTo(menu.left - 4, 0.01),
      reason: 'submenu $sub did not flip to the left of $menu',
    );
  });

  testWidgets('quick cells and grid cells run their action after the pop, like rows', (
    tester,
  ) async {
    await pumpHost(tester);
    String? ran;
    ...
    expect(
      ...
      closeTo(tester.getTopLeft(find.text('Before')).dy, 0.01),
    );
    expect(
      tester.getTopLeft(find.text('First frame')).dy,
      greaterThan(tester.getTopLeft(find.text('Before')).dy),
    );
    expect(
      tester.getTopLeft(find.text('First frame')).dx,
      closeTo(tester.getTopLeft(find.text('Before')).dx, 0.01),
    );
