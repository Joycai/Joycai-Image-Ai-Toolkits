...
void main() {
  ...
  group('inferPlatform', () {
    test('a vendor-specific id names its platform whatever the host', () {
      ...
      cases.forEach((vendor, platform) {
        expect(Platforms.inferPlatform(vendor, 'https://relay.example.com/v1').id,
            platform,
            reason: vendor);
      });
    });
    ...
    test('a generic vendor is claimed by the host it points at', () {
      ...
      expect(
          Platforms.inferPlatform(
                  Vendors.openAIResponsesRest, 'https://API.OpenAI.com/v1')
              .id,
          Platforms.openai);
      expect(
          Platforms.inferPlatform(
                  Vendors.anthropicRest, 'https://api.anthropic.com/v1')
              .id,
          Platforms.anthropic);
      expect(
          Platforms.inferPlatform(Vendors.googleRest,
                  'https://generativelanguage.googleapis.com/v1beta')
              .id,
          Platforms.google);
      // Google's OpenAI-compatible face is still Google.
      expect(
          Platforms.inferPlatform(Vendors.openAIRest,
    ...
    test('anything else is custom, including a non-URL endpoint', () {
      for (final vendor in [
        ...
        expect(Platforms.inferPlatform(vendor, 'https://relay.example.com/v1').id,
            Platforms.custom,
            reason: vendor);
        expect(Platforms.inferPlatform(vendor, 'relay.example.com').id,
            Platforms.custom,
            reason: vendor);
      }
    });
  ...
  group('routeVendor', () {
    test('a face the primary vendor already offers stays with it', () {
      ...
      expect(Platforms.routeVendor(ds, Vendors.dashscopeNative, RouteKind.chat),
          Vendors.dashscopeNative);
      expect(
          Platforms.routeVendor(
              ds, Vendors.dashscopeNative, RouteKind.anthropic),
          Vendors.dashscopeNative);
      expect(Platforms.routeVendor(ds, Vendors.dashscope, RouteKind.dashscope),
          Vendors.dashscope);
    });
    ...
    test('a face the primary vendor never offered takes the platform vendor',
      ...
      expect(
          Platforms.routeVendor(
              newapi, Vendors.newApiGemini, RouteKind.anthropic),
          Vendors.newApiAnthropic);
      final mm = Platforms.byId(Platforms.minimax);
      expect(Platforms.routeVendor(mm, Vendors.minimax, RouteKind.anthropic),
          Vendors.minimaxAnthropic);
    });

    test('a face neither offers resolves to null', () {
      expect(
          Platforms.routeVendor(Platforms.byId(Platforms.deepseek),
              Vendors.deepseek, RouteKind.gemini),
          isNull);
    });
  });
