...
void main() {
  ...
  testWidgets('toggleLeftPanel closes a drawer it opened', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(key.currentState!.isDrawerOpen, isFalse,
        reason: 'the second press hides it — `openDrawer` on an open drawer '
            'is a no-op, which is what left the chord dead');
  });

  testWidgets('toggleRightPanel closes an end drawer it opened',
      (tester) async {
    final key = await mountScaffold(tester);
    final layout = layoutFor(key);
  ...
  testWidgets('the phone sheet has no close, and says so by opening',
      (tester) async {
    final key = await mountScaffold(tester);
    var opened = 0;
