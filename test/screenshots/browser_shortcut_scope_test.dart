...
void main() {
  ...
  testWidgets('the tree\'s rename field keeps the keys the grid would take', (
    ...
    await settle(tester);
    await tester.tap(find.descendant(of: find.byType(AppGlassMenu), matching: find.text('重命名')));
    await settle(tester);
    ...
    final LogicalKeyboardKey mod = Platform.isMacOS
  ...
  Future<void> clickFolderThenFile(WidgetTester tester) async {
    ...
    expect(
      ...
      'browser-pane-grid',
      reason:
          'and clicking the grid has to take it back — before this '
  ...
  testWidgets('Delete after clicking a folder deletes the files, not the folder', (
    ...
    expect(
      ...
      findsNothing,
      reason:
          'Delete belongs to the pane that has the keyboard, and that '
          'is the grid',
    );
    expect(
    ...
    expect(
      find.descendant(of: find.byType(AppDialog), matching: find.text(picked.name)),
      findsOneWidget,
  ...
  testWidgets('F2 after clicking a folder renames the file, not the folder', (
    ...
    expect(
      find.descendant(of: find.byType(DirectoryTreeItem), matching: find.byType(TextField)),
      findsNothing,
      reason: 'F2 used to open the tree\'s inline folder-name editor',
    );
    expect(
      find.byType(FileRenameDialog),
      findsOneWidget,
  ...
  testWidgets('Cmd+F during an inline rename does not commit the half-typed name', (
    ...
    expect(
      ...
      isTrue,
      reason:
          'and still hold the keyboard — no *key* may pull focus out of '
    ...
    expect(
      Directory(p.join(env.root.path, 'half-typed')).existsSync(),
      isFalse,
  ...
  testWidgets('the context menu spells its keys from the table', (WidgetTester tester) async {
    await mountApp(
      tester,
      env: env,
      screen: AppScreen.fileBrowser,
    ...
    await tester.tap(
      find.ancestor(of: find.text('photo_1.png'), matching: find.byType(FileCard)),
      buttons: kSecondaryButton,
    );
    await settle(tester);

    final AppLocalizations l10n = AppLocalizations.of(tester.element(find.byType(AppGlassMenu)))!;
    for (final id in <String>[
      ...
      expect(
        find.descendant(of: find.byType(AppGlassMenu), matching: find.text(hint!)),
        findsWidgets,
        reason:
            '$id must show the key it really has, not a literal — the '
    ...
    expect(
      find.descendant(of: find.byType(AppGlassMenu), matching: find.text(l10n.rename)),
      findsOneWidget,
    );
  ...
  testWidgets('Cmd+\\ hides and shows the folder column', (WidgetTester tester) async {
    ...
    expect(find.byType(UnifiedSidebar), findsNothing, reason: 'the column is gone');
  ...
  testWidgets('Shift+Cmd+\\ opens and closes the staging column', (WidgetTester tester) async {
    await mountApp(
      tester,
      env: env,
      screen: AppScreen.fileBrowser,
    ...
    expect(openAtStart, isFalse, reason: 'the fixture stages nothing and the column starts closed');

    await pressChord(tester, LogicalKeyboardKey.backslash, shift: true);
    expect(find.byType(BrowserStagingPanel), openAtStart ? findsNothing : findsOneWidget);
  ...
  testWidgets('Shift+Cmd+C copies every selected name, one per line', (WidgetTester tester) async {
    String? copied;
