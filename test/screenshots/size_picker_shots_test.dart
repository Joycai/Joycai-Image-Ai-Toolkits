...
void main() {
  ...
  const rates = [SpecRate(size: '1K', price: 0.2), SpecRate(size: '2K', price: 0.4), SpecRate(size: '4K', price: 0.8)];
  ...
  for (final brightness in Brightness.values) {
    testWidgets('size picker · ${brightness.name}', (tester) async {
      ...
      await tester.pumpAndSettle();
      await tester.enterText(find.descendant(of: find.byKey(gpt), matching: find.byType(TextField)).first, '4:1');
      ...
      await expectLater(find.byType(Scaffold), matchesGoldenFile('size_picker_${brightness.name}.png'));
    });
  }
}
