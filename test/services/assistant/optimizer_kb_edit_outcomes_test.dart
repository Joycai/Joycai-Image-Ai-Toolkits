...
void main() {
  ...
  test('decided edits are reported once, at the start of the next turn', () async {
    ...
    final applied = session.stageKbEditForTest(
        relPath: 'rules/a.md', newContent: 'new', oldContent: 'old');
    final rejected = session.stageKbEditForTest(
        relPath: 'rules/b.md', newContent: 'new', oldContent: null);
    ...
    expect(note.content, isNot(contains('rules/c.md')), reason: 'still pending — nothing to report');
  ...
  test('the record stays in the history but is neither a chat line nor a user turn', () async {
    ...
    expect(session.history.where((m) => m.content.contains('rules/a.md')), hasLength(1),
    ...
    expect(
      restored.transcript.where((e) => e.kind == OptimizerEntryKind.user).map((e) => e.text),
      ['first', 'second'],
    );
  });
}
