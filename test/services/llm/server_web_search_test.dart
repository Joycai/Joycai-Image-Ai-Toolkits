...
void main() {
  ...
  group('LLMDispatcher.serverWebSearch', () {
    test('④ vendors declare a search tool whose sources come back', () {
      expect(support(Vendors.anthropicRest, 'claude-opus-5'), ServerWebSearch.withSources);
      expect(support(Vendors.minimaxAnthropic, 'MiniMax-M3'), ServerWebSearch.withSources);
    });
    ...
    test("Bailian's ① and native faces take the traceless switch", () {
      ...
      expect(support(Vendors.dashscopeNative, 'qwen3-max'), ServerWebSearch.traceless);
      expect(
        support(Vendors.dashscope, 'qwen3-max', wireProtocol: 'dashscope-chat'),
        ServerWebSearch.traceless,
      );
      expect(
        support(Vendors.dashscopeNative, 'qwen3-max', wireProtocol: 'openai-chat'),
        ServerWebSearch.traceless,
      );
    });
    ...
    test('everywhere else there is nothing to switch', () {
      ...
      expect(support(Vendors.deepseek, 'deepseek-v4-pro'), ServerWebSearch.unsupported);
      expect(support(Vendors.officialGoogle, 'gemini-3-pro-preview'), ServerWebSearch.unsupported);
      expect(support(Vendors.midjourneyProxy, 'midjourney'), ServerWebSearch.unsupported);
    });

    test('a model that does not go down chat has none', () {
      expect(
        support(Vendors.anthropicRest, 'claude-opus-5', tag: 'image'),
        ServerWebSearch.unsupported,
      );
      expect(support(Vendors.dashscope, 'qwen-image-3.0'), ServerWebSearch.unsupported);
    });
  });

  LLMTarget target(String channelType, String modelId, {bool webSearch = true, String? endpoint}) {
  ...
  group('① compatible face', () {
    Map<String, dynamic> payload(LLMTarget t) =>
        OpenAIChatProtocol().buildChatPayloadForTest(t, hi, isStreaming: false);

    test('a declared face sends top-level enable_search', () {
      expect(payload(target(Vendors.dashscope, 'qwen3-max'))['enable_search'], isTrue);
      expect(payload(target(Vendors.dashscopeNative, 'qwen3-max'))['enable_search'], isTrue);
    });

    test('switched off, nothing is sent', () {
      expect(
        payload(
          target(Vendors.dashscope, 'qwen3-max', webSearch: false),
    ...
    test('an undeclared vendor never sends it, whatever the stored flag says', () {
      ...
      for (final id in [Vendors.openAIRest, Vendors.deepseek, Vendors.minimax]) {
        expect(
          payload(
            target(id, 'm', endpoint: 'https://api.example.com/v1'),
          ).containsKey('enable_search'),
          isFalse,
          reason: id,
        );
  ...
  group('native face', () {
    Map<String, dynamic> body(LLMTarget t) =>
        buildDashScopeChatPayload(t, hi, multimodal: false, isStreaming: false);
  ...
  group('④ face', () {
    ...
    test('a ④ vendor sends the server tool', () {
      final declared = tools(
        target(Vendors.anthropicRest, 'claude-opus-5', endpoint: 'https://api.anthropic.com'),
      );
    ...
    test('the payload and the editor ask the same question', () {
      for (final id in [Vendors.anthropicRest, Vendors.minimaxAnthropic, Vendors.dashscope]) {
        ...
        expect(
          vendor.sendsWebSearchOn(WireProtocol.anthropicChat),
  ...
  group('a call that opts out of server tools', () {
    ...
    test('sends no search even when the model has it on', () {
      ...
      expect(
        payload(forCall(t, const {llmNoServerToolsKey: true})).containsKey('enable_search'),
        isFalse,
      );
      final anthropic = target(
        Vendors.anthropicRest,
        'claude-opus-5',
        endpoint: 'https://api.anthropic.com',
      );
      final body = prepareAnthropicPayload(
        forCall(anthropic, const {llmNoServerToolsKey: true}),
        hi,
        isStreaming: false,
      );
    ...
    test('any other call keeps the model switch', () {
      final t = target(Vendors.dashscope, 'qwen3-max');
      expect(payload(forCall(t, const {'retryCount': 2}))['enable_search'], isTrue);
