...
void main() {
  ...
  group('Responses-led channel types', () {
    test('lead with Responses and keep Chat Completions on the menu', () {
      ...
      for (final id in chatLed.values) {
        expect(Vendors.byId(id).menuFor(Surface.chat).first, WireProtocol.openaiChat, reason: id);
      }
    });
    ...
    test('route an unpinned chat model to Responses', () {
      for (final id in responsesLed) {
        expect(
          LLMDispatcher.autoProtocolFor(id, 'gpt-5.6-sol'),
          WireProtocol.openaiResponses,
          reason: id,
        );
        expect(
    ...
    test('still accept a Chat Completions pin', () {
      for (final id in responsesLed) {
        expect(
          LLMDispatcher.isStaleProtocolSelection(id, 'gpt-5.6-sol', WireProtocol.openaiChat.id),
          isFalse,
          reason: id,
        );
        expect(
    ...
    test('leave image and video routes where the chat-led sibling has them', () {
      for (final entry in chatLed.entries) {
        for (final (model, tag) in [
          ...
          expect(
            LLMDispatcher.autoProtocolFor(entry.key, model, tag: tag),
            LLMDispatcher.autoProtocolFor(entry.value, model, tag: tag),
            reason: '$model on ${entry.key}',
          );
          expect(
            LLMDispatcher.protocolMenu(entry.key, model, tag: tag).options,
  ...
  group('presets', () {
    test('OpenAI official switches between the two chat interfaces', () {
      final responses = presetForChannelType(
        Vendors.openAIResponsesRest,
        endpoint: 'https://api.openai.com/v1',
      );
      expect(responses?.id, 'openai-official');
      expect(variantForChannelType(responses!, Vendors.openAIResponsesRest)?.id, 'responses');

      final chat = presetForChannelType(Vendors.openAIRest, endpoint: 'https://api.openai.com/v1');
      ...
      expect(chat.variants.map((v) => v.defaultEndpoint).toSet(), {'https://api.openai.com/v1'});
    });

    test('New API offers a Responses format on the same /v1 suffix', () {
      final preset = presetForChannelType(
