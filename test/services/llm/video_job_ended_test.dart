...
void main() {
  Matcher ended(bool value) => throwsA(isA<LLMApiException>()
      .having((e) => e.isJobEnded, 'isJobEnded', value));
    ...
    for (final status in ['failed', 'cancelled', 'expired']) {
      expect(
          () => openaiVideoPollEnvelope(
              {'status': status}, 'op', 'https://api.example.com/v1'),
          ended(true),
          reason: status);
    }
    expect(
        openaiVideoPollEnvelope(
            {'status': 'in_progress'}, 'op', 'https://api.example.com/v1')['done'],
        isNot(true));
  });
  ...
  test('xAI: failed, expired, cancelled and done-without-URL are ended', () {
    for (final data in [
      ...
      expect(() => xaiVideoPollEnvelope(data, 'op', 'https://api.x.ai/v1'),
          ended(true),
          reason: '$data');
    }
  });
  ...
  test('Veo: an operation error and a safety filter are ended', () {
    expect(
        () => veoPollResult({
            ...
            }, 'op', 'https://generativelanguage.googleapis.com/v1beta'),
        ended(true));
    expect(
        () => veoPollResult({
              'done': true,
  ...
  test('MiniMax H3 local: failed is ended', () {
    expect(
        () => minimaxH3PollEnvelope(
            {'status': 'failed', 'error': 'oom'}, 'op', 'http://localhost/c'),
        ended(true));
  });
}
