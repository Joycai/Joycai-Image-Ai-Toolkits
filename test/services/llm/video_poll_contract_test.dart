...
void main() {
  ...
  group('videoUriNeedsAuth', () {
    test('only a URL back on the API host gets the key', () {
      expect(
        videoUriNeedsAuth(
          'https://relay.example.com/v1/videos/v1/content',
    ...
    test('same host on another scheme or port never receives the key', () {
      ...
      expect(videoUriNeedsAuth('http://relay.example.com/video.mp4', endpoint), isFalse);
      expect(videoUriNeedsAuth('https://relay.example.com:8443/video.mp4', endpoint), isFalse);
      expect(videoUriNeedsAuth('https://relay.example.com:443/video.mp4', endpoint), isTrue);
    });
  });
  ...
  group('openaiVideoPollEnvelope', () {
    ...
    test('completed without a url falls back to /content, which needs auth', () {
      final env = openaiVideoPollEnvelope({'status': 'completed'}, 'video_1', base);
    ...
    test('a 2xx body without status fails immediately as malformed', () {
      expect(
        ...
        throwsA(isA<LLMApiException>().having((e) => e.message, 'message', contains('no status'))),
      );
    });
  });
  ...
  group('veoPollResult (B6)', () {
    ...
    test('a safety-filtered result says so', () {
      expect(
        ...
        throwsA(isA<LLMApiException>().having((e) => e.message, 'message', contains('celebrity'))),
      );
    });
      ...
      final data = veoPollResult(
        {
          ...
          'response': {
            'generateVideoResponse': {
              'generatedSamples': [
                {
                  'video': <String, dynamic>{'uri': 'https://files.relay-cdn.net/v.mp4'},
                },
              ],
            },
          },
