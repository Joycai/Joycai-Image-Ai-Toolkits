...
void main() {
  ...
  List<LLMMessage> ofChars(int chars, {int images = 0}) => [
    ...
    LLMMessage(
      ...
      content: 'x' * (chars - 10),
      attachments: [
        for (var i = 0; i < images; i++) LLMAttachment.fromFile(File('img_$i.png'), 'image/png'),
      ],
    ),
  ];
  ...
  group('ContextBudget preflight', () {
    ...
    test('the estimate is a floor: 6 chars per token, 258 per image', () {
      ...
      expect(
        ContextBudget.estimateRequestTokens(chars: 6000, toolSchemaChars: 600, images: 2),
        1100 + 516,
      );
    });
    ...
    test('no budget the assistant can reach trips it', () {
      ...
      for (final window in [2048, 4096, 32768, 131072, 1000000]) {
        final maxOccupancy =
            ContextBudget.readCapChars(window, 0, observedCharsPerToken: 6.0) +
            ContextBudget.reserveFor((window * 6.0).round());
        final estimate = ContextBudget.estimateRequestTokens(chars: maxOccupancy);
        expect(ContextBudget.exceedsWindow(estimate, window), isFalse, reason: 'window $window');
      }
    });
  });
  ...
  group('LLMService.preflightContextSize', () {
    test('a request clearly over an explicit window is refused with both '
        'numbers', () {
      final error = LLMService.preflightContextSize(config(4096), ofChars(60000), null);
    ...
    test('a request that fits is sent', () {
      expect(LLMService.preflightContextSize(config(4096), ofChars(20000), null), isNull);
    });

    test('unset and unlimited windows are never checked', () {
      expect(LLMService.preflightContextSize(config(null), ofChars(10000000), null), isNull);
      expect(LLMService.preflightContextSize(config(0), ofChars(10000000), null), isNull);
    });

    test('images and tool schemas count toward the estimate', () {
      // 24 000 chars = 4000 tokens: fits a 4096 window on its own…
      expect(LLMService.preflightContextSize(config(4096), ofChars(24000), null), isNull);
      // …but not with four images (+1032) on top.
      expect(
        LLMService.preflightContextSize(config(4096), ofChars(24000, images: 4), null),
        isNotNull,
      );
      ...
      expect(LLMService.preflightContextSize(config(4096), ofChars(24000), [bigTool]), isNotNull);
    });
    ...
    test('tool-call arguments in the history are counted', () {
      final history = [
        LLMMessage(
          ...
          content: '',
          toolCalls: [
            LLMToolCall(id: 't1', name: 'write_knowledge_file', arguments: {'body': 'y' * 60000}),
          ],
        ),
      ];
      expect(LLMService.preflightContextSize(config(4096), history, null), isNotNull);
    });
  });
