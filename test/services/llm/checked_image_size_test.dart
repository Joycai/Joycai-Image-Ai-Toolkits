...
void main() {
  ...
  test('a size the model does not take becomes its default, logged', () {
    final logs = <String>[];
    final checked = optionsWithCheckedSize(
      target('gpt-image-1'),
      const {'imageSize': '4096x4096', 'quality': 'high'},
  ...
  test('a free size is judged by the model rules', () {
    ...
    expect(
        optionsWithCheckedSize(target('gpt-image-2'), const {'imageSize': '3840x3840'})![
            'imageSize'],
        isNot('3840x3840'));
    expect(
        optionsWithCheckedSize(target('gpt-image-2'), const {'imageSize': '2048x1152'})![
            'imageSize'],
        '2048x1152');
  });
