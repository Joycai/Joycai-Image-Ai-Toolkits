...
void main() {
  group('content blocks fail the request (pitfalls 11 §A7, errors 06 §2.3)', () {
    test('① content_filter is a typed, non-retryable failure', () {
      ...
      expect(
        LLMService.isRetryable(failure),
        isFalse,
    ...
    test('ordinary endings are not blocks', () {
      for (final reason in ['stop', 'length', 'tool_calls', 'pause', null]) {
        expect(contentBlockedFailure({'finish_reason': reason}), isNull, reason: '$reason');
      }
  ...
  group('error messages name the request URL (errors 06 §2)', () {
    ...
    final request = http.Request(
      'POST',
      Uri.parse(
        'https://relay.example.com/v1beta/models/m:generateContent'
        '?key=SECRET-KEY',
      ),
    );
      ...
      expect(
        ...
        throwsA(
          isA<LLMApiException>().having(
            (e) => e.message,
            'message',
            startsWith('Test API request failed: 502 - '),
          ),
        ),
      );
    });
