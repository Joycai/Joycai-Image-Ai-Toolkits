...
void main() {
  group('geminiChunksFromSseLine (B1)', () {
    const chunk = '{"candidates":[{"content":{"parts":[{"text":"hi"}],"role":"model"}}]}';
    ...
    test('an in-chunk error envelope still throws', () {
      expect(
        () => geminiChunksFromSseLine('data: {"error":{"code":429,"message":"quota"}}').toList(),
        throwsA(isA<LLMApiException>().having((e) => e.isEnvelope, 'isEnvelope', isTrue)),
      );
    });
  });
  ...
  group('decodeJsonBody (B2)', () {
    ...
    test('2xx non-JSON body names the real problem instead of crashing', () {
      ...
      expect(
        () => decodeJsonBody(response),
        throwsA(isA<LLMApiException>().having((e) => e.message, 'message', contains('non-JSON'))),
      );
    });
    ...
    test('201/202 are inside the success window (submit/poll surfaces)', () {
      ...
      expect(decodeJsonBody(http.Response('{"status":"pending"}', 202)), {'status': 'pending'});
    });
    ...
    test('checkEnvelope: false hands a failed-job body to the caller', () {
      ...
      final response = http.Response('{"status":"failed","error":{"message":"boom"}}', 200);
      expect(decodeJsonBody(response, checkEnvelope: false), containsPair('status', 'failed'));
      expect(() => decodeJsonBody(response), throwsA(isA<LLMApiException>()));
    });
  });
  ...
  group('VendorProfile.downloadHeaders', () {
    test('google-keyed vendors get x-goog-api-key, bearer vendors a bearer', () {
      expect(Vendors.byId(Vendors.officialGoogle).downloadHeaders('k'), {'x-goog-api-key': 'k'});
      expect(Vendors.byId(Vendors.googleRest).downloadHeaders('k'), {'x-goog-api-key': 'k'});
      expect(Vendors.byId(Vendors.openAIRest).downloadHeaders('k'), {'Authorization': 'Bearer k'});
  ...
  group('LLMService.isRetryable (B5)', () {
    test('structured 5xx and 429 retry; other codes do not', () {
      expect(LLMService.isRetryable(LLMApiException('x', statusCode: 502)), isTrue);
      expect(LLMService.isRetryable(LLMApiException('x', statusCode: 429)), isTrue);
      expect(LLMService.isRetryable(LLMApiException('x', statusCode: 400)), isFalse);
      expect(LLMService.isRetryable(LLMApiException('x', statusCode: 401)), isFalse);
    });

    test('envelope errors never retry — the transport succeeded', () {
      expect(
        LLMService.isRetryable(
          LLMApiException('API error in response body: quota', isEnvelope: true),
        ),
        isFalse,
      );
    });
    ...
    test('numbers in error prose are not status codes', () {
      ...
      expect(
        LLMService.isRetryable(
          Exception('API error in response body: rate limited, retry after 500ms'),
        ),
        isFalse,
      );
      expect(LLMService.isRetryable(Exception('model "gpt-500-turbo" does not exist')), isFalse);
    });

    test('legacy "... failed: <status>" prose still recognized', () {
      expect(
        LLMService.isRetryable(Exception('xAI Images API failed: 503 - upstream down')),
        isTrue,
      );
      expect(LLMService.isRetryable(Exception('xAI Images API failed: 422 - bad prompt')), isFalse);
    });
  });
  ...
  group('the non-streaming deadline', () {
    ...
    test('a generation that ran long is never retried', () {
      ...
      expect(LLMService.isRetryable(const LLMDeadlineExceeded(Duration(seconds: 120))), isFalse);
    });
    ...
    test('says what happened and that nothing was retried', () {
      final message = const LLMDeadlineExceeded(Duration(seconds: 388)).toString();
    ...
    test('a caller that knows its answer is long is believed over the guess', () {
      ...
      final dispatcher = LLMDispatcher();
      final guessed = dispatcher.generateTimeout(config('gpt-4o', Vendors.openAIRest));
      ...
      expect(told, dispatcher.generateTimeout(config('claude-opus-4-8', Vendors.anthropicRest)));
    });
  ...
  group('tool calls over the streaming surface', () {
    ...
    test('④ carries them, so a tool-bearing request may stream', () {
      ...
      expect(LLMDispatcher().streamSupportsTools(config(Vendors.anthropicRest)), isTrue);
      expect(LLMDispatcher().streamSupportsTools(config(Vendors.newApiAnthropic)), isTrue);
    });
    ...
    test('③ carries them too — its parser is shared with the sync path', () {
      ...
      expect(LLMDispatcher().streamSupportsTools(config(Vendors.googleRest)), isTrue);
      expect(LLMDispatcher().streamSupportsTools(config(Vendors.officialGoogle)), isTrue);
    });
    ...
    test('① carries them since it grew an accumulator', () {
      ...
      for (final vendor in [Vendors.openAIRest, Vendors.newApiOpenAI]) {
        expect(
          LLMDispatcher().streamSupportsTools(config(vendor)),
          isTrue,
    ...
    test('Midjourney keeps the downgrade — it has no accumulator', () {
      ...
      expect(LLMDispatcher().streamSupportsTools(config(Vendors.midjourneyProxy)), isFalse);
    });
