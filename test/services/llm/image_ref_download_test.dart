...
void main() {
  ...
  test('a failed link is retried once and the second answer used', () async {
    ...
    final client = MockClient((_) async {
      calls++;
      return calls == 1 ? http.Response('bad gateway', 502) : http.Response.bytes(png, 200);
    });
  ...
  test('an HTML 200 is not an image, even after the retry', () async {
    ...
    expect(bytes, isNull);
    expect(calls, 2);
    expect(logs.any((l) => l.startsWith('WARN') && l.contains('not an image')), isTrue);
  });
  ...
  test('a data: URI in a url field is decoded, not fetched', () async {
    ...
    final bytes = await resolveImageRef('data:image/png;base64,${base64Encode(png)}', client, null);
    expect(bytes, png);
  });
  ...
  test('inline data that is not an image is refused', () async {
    ...
    expect(
      await resolveImageRef(base64Encode(utf8.encode('{"error":"nope"}')), client, null),
      isNull,
    );
  });
  ...
  test('line-wrapped base64 still decodes', () async {
    final client = MockClient((_) async => fail('no fetch'));
    final wrapped = base64Encode(png).replaceAllMapped(RegExp(r'.{4}'), (m) => '${m.group(0)}\n');
    expect(await resolveImageRef(wrapped, client, null), png);
  });

  test('a partial result is delivered with a WARN naming the shortfall', () async {
    final logs = <String>[];
    final client = MockClient(
      (req) async => req.url.path.endsWith('ok.png')
    ...
    expect(logs.any((l) => l.startsWith('WARN') && l.contains('only 1 of 2')), isTrue);
  });
