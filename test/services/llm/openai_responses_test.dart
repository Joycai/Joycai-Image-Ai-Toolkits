...
void main() {
  ...
  group('request builder', () {
    ...
    test('tool_choice keeps the keywords and names a function unwrapped', () {
      ...
      expect(responsesToolChoice('list_files'), {'type': 'function', 'name': 'list_files'});
      final body = payload([user], tools: [tool], options: {'toolChoice': 'list_files'});
    ...
    test('reasoning: default sends nothing, off is none, levels carry summary', () {
      ...
      expect(payload([user], t: target(effort: ReasoningEffort.high))['reasoning'], {
        'effort': 'high',
        'summary': 'auto',
      });
      expect(payload([user], t: target(effort: ReasoningEffort.max))['reasoning'], {
        'effort': 'max',
        'summary': 'auto',
      });
    });
    ...
    test('include is sent only where the vendor declares it', () {
      ...
      expect(
        payload([user], t: target(channelType: Vendors.newApiOpenAI)).containsKey('include'),
        isFalse,
      );
      expect(payload([user], t: target(channelType: Vendors.xaiApi))['include'], [
  ...
  group('history conversion', () {
    ...
    test('the same model gets the items verbatim, instead of bare calls', () {
      final input = buildResponsesInput([
        user,
        toolTurn(producer: 'gpt-5.5'),
        result,
      ], modelId: 'gpt-5.5');
      expect(input.sublist(1, 3), items);
      expect(
        input.where((i) => i['type'] == 'function_call').length,
        1,
        reason: 'never both the verbatim call and a rebuilt one',
      );
      expect(input.any((i) => i['role'] == 'assistant'), isFalse);
      expect(input.last, {'type': 'function_call_output', 'call_id': 'call_1', 'output': 'a.txt'});
    });
    ...
    test('items with no recorded producer are not replayed', () {
      final input = buildResponsesInput([user, toolTurn(), result], modelId: 'gpt-5.5');
      expect(input.any((i) => i['type'] == 'reasoning'), isFalse);
    });
  ...
  group('stream assembler', () {
    List<LLMResponseChunk> run(List<Map<String, dynamic>> events, {String? sentEffort}) {
    ...
    Map<String, dynamic> completed({Map<String, dynamic>? usage, Map? reasoning}) => {
      ...
      'response': {'status': 'completed', 'usage': ?usage, 'reasoning': ?reasoning},
    };

    String textOf(List<LLMResponseChunk> c) => c.map((x) => x.textPart ?? '').join();
    ...
    test('text, reasoning summary and usage in the recorder shape', () {
      ...
      expect(
        LLMService.promptTokensOf(meta),
        100,
    ...
    test('parallel calls group by output_index, without item_id', () {
      final chunks = run([
        {
          ...
          'output_index': 0,
          'item': {'type': 'function_call', 'call_id': 'c0', 'name': 'a'},
        },
        {
          'type': 'response.output_item.added',
          'output_index': 1,
      ...
      expect(
        chunks.any((c) => c.rawResponseItems != null),
        isFalse,
    ...
    test('the whole string wins over the accumulated deltas', () {
      ...
      expect(chunks.firstWhere((c) => c.toolCallPart != null).toolCallPart!.arguments, {'x': 7});
    });
    ...
    test('replay items come from output_item.done, server tools left out', () {
      ...
      expect(
        chunks.map((c) => c.reasoningPart ?? '').join(),
        'think',
    ...
    test('a streamed refusal is content_filter, never reply text', () {
      ...
      expect(textOf(chunks), 'Sure, ', reason: 'the refusal text must not reach the deliverable');
      final meta = metaOf(chunks);
      expect(meta['finish_reason'], contentFilterFinishReason);
    ...
    test('failed, error event and a bare error all throw', () {
      ...
      expect(
        ...
        throwsA(isA<LLMApiException>().having((e) => e.message, 'message', contains('slow down'))),
      );
      expect(
        () => ResponsesStreamAssembler().feed({
          'error': {'message': 'quota'},
    ...
    test('no terminal event with a call in flight throws', () {
      ...
      a.feed({
        'type': 'response.output_item.added',
        'output_index': 0,
        'item': {'type': 'function_call', 'call_id': 'c', 'name': 'a'},
      });
      a.feed({
        'type': 'response.function_call_arguments.delta',
        'output_index': 0,
    ...
    test('an echoed effort that differs is reported, never missing echoes', () {
      ...
      expect(metaOf(run(events, sentEffort: 'none')).containsKey('wire_rewrites'), isFalse);
      expect(
        metaOf(run(events)).containsKey('wire_rewrites'),
        isFalse,
        reason: 'nothing was sent, so nothing was rewritten',
      );
      final noEcho = [events.first, completed()];
      expect(metaOf(run(noEcho, sentEffort: 'max')).containsKey('wire_rewrites'), isFalse);
    });
  });
      ...
      final response = responsesResponseFromBody({
        ...
        'output': [
          ...
          },
          {
            ...
            'role': 'assistant',
            'content': [
              {'type': 'output_text', 'text': 'Doing it.'},
            ],
          },
          fc,
        ],
  ...
  group('carrier', () {
    ...
    test('survives persistence and is absent when null', () {
      ...
      expect(
        LLMMessage(role: LLMRole.assistant, content: 'x').toJson().containsKey('rawResponseItems'),
        isFalse,
      );
    });
  ...
  group('routing', () {
    test('the Responses face is an alternate after ① on the generic OpenAI hosts', () {
      for (final id in [Vendors.openAIRest, Vendors.newApiOpenAI]) {
        ...
        expect(menu.options, [WireProtocol.openaiChat, WireProtocol.openaiResponses], reason: id);
        expect(menu.auto, WireProtocol.openaiChat, reason: id);
        expect(
          protocolSectionForm(menu, pinIsStale: false),
      ...
      expect(xai.options, [WireProtocol.openaiResponses, WireProtocol.openaiChat]);
      expect(xai.auto, WireProtocol.openaiResponses);
      expect(protocolSectionForm(xai, pinIsStale: false), ProtocolSectionForm.dropdown);
      expect(
        LLMDispatcher.isStaleProtocolSelection(
          Vendors.xaiApi,
          'grok-4.5',
          WireProtocol.openaiChat.id,
        ),
        isFalse,
      );
      ...
      expect(
        LLMDispatcher.isStaleProtocolSelection(
          Vendors.deepseek,
          'deepseek-chat',
          WireProtocol.openaiResponses.id,
        ),
        isTrue,
      );
      expect(
    ...
    test('a Responses model streams tools; the face follows default and pin', () {
      ...
      expect(
        LLMDispatcher.resolvedChatFace(channelType: Vendors.xaiApi, modelId: 'grok-4.5'),
      ...
      expect(
        LLMDispatcher.resolvedChatFace(channelType: Vendors.openAIRest, modelId: 'gpt-5.5'),
        WireProtocol.openaiChat,
      );
      expect(
        LLMDispatcher.resolvedChatFace(
