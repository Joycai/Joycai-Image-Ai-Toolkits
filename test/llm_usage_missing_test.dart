...
void main() {
  ...
  test('token billing with no usage at all is flagged', () {
    ...
    expect(
        LLMService.usageMissing(config('token'), const {'finish_reason': 'stop'}),
        isTrue);
    expect(
        LLMService.usageMissing(config('token'),
  ...
  test('any reported count is usage', () {
    expect(
        LLMService.usageMissing(config('token'), const {'prompt_tokens': 12}),
        isFalse);
    expect(
        LLMService.usageMissing(
            config('token'), const {'candidatesTokenCount': 3}),
        isFalse);
    expect(
        LLMService.usageMissing(config('token'), const {'output_tokens': 5}),
        isFalse);
  });
