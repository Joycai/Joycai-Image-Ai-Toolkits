...
void main() {
  ...
  testWidgets(
    'opens with the stem selected, the extension locked beside it, and the hint row',
    (tester) async {
  ...
  testWidgets(
    'a name already in the folder disables Rename and says so in place of the hint',
    (tester) async {
      await pumpHost(tester);
      await open(tester);
      ...
      expect(
        find.text('hero_final.png already exists in this folder'),
        findsOneWidget,
      );
  ...
  testWidgets('illegal characters and an empty name are refused the same way', (
    tester,
  ) async {
    await pumpHost(tester);
    await open(tester);
  ...
  testWidgets('Enter on a usable name renames the file and reports back', (
    tester,
  ) async {
    await pumpHost(tester);
    var succeeded = false;
