...
**创建 → 轮询 → 取结果**整条链路跑通（`MiniMax-H3`）。同时被这一次确认的还有
四件事：`/v2` base 从渠道存的地址推导正确；`resolution` / `duration` 两个必填项
的客户端缺省值（§6.3）被上游接受；轮询响应到本项目 Veo 形状信封的映射；结果
直链可下载。这条 wire 从此不再是纸面接法。

**仍未实测**：

- **§4.1 的媒体项嵌套形状**（`{"type":"image_url","image_url":{"url":…}}`）。
  纯文生视频的 `content[]` 里没有媒体项，跑通它不经过这条。首尾帧或参考素材
  的路径才会——而这正是 2026-08-29 复核时改对的那处，改之前是平铺的 `url`。
