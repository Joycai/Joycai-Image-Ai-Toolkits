...
**实测**（2026-08-29，用户账号，`api.minimaxi.com`）：V1 视频面的
**创建 → 轮询 → 取结果**整条链路跑通（`MiniMax-H3`，**首帧 + 尾帧**模式）。
被这一次确认的：

1. **§4.1 的媒体项嵌套形状** ——
   `{"type":"image_url","image_url":{"url":…},"role":"first_frame"}`。
   这是 2026-08-29 复核时改对的那处，改之前写的是平铺的 `url`。
   **它的失败模式是静默的**：平铺形状照样被解析器接受、任务照样成功、
   照样计费，只是生成时当没附图——所以"跑通了"这句话在这条上的判据不是
   HTTP 200，而是**出片确实遵守了首尾帧**。
2. `role` 与 `type` 平级、`first_frame` / `last_frame` 两个拼写，以及同一个
   `content[]` 里放两个媒体项。
3. 媒体项旁边那个**必须存在**的 text 项（纯图生视频也要给）。
4. `ratio: adaptive` 在带媒体时不被替换的那条分支。
5. `resolution: 2K` 被接受并正常出片。它实际出多少像素没记录，
   但这不是个缺口：本项目的选项直接用 MiniMax 自己的档位拼写，
   UI 不承诺尺寸，所以像素数不是承重的事实。
6. `/v2` base 从渠道存的地址推导正确；`resolution` / `duration` 两个必填项
   的客户端缺省值（§6.3）被上游接受；轮询响应到本项目 Veo 形状信封的映射；
   结果直链可下载。
...
- **纯文生视频的 `ratio` 替换**（`adaptive` → `16:9`，§4.1）。上面跑的是带
  媒体的分支，走的正好是它的反面。
- **首尾帧与参考素材并存时的互斥裁剪**（`partitionMiniMaxVideoMedia`，
  本项目让帧胜出）。这次只给了帧，没有同时给参考素材。
- `reference_image` 这个 role 本身。
- **§4.4 的取消（DELETE）**。要在 `queued` 窗口内点中才走得到，且只对 `queued`
  有效（§6.6）。
- `768P` 档（本项目对不认识的值的回退值）。上面确认的是 `2K`。
