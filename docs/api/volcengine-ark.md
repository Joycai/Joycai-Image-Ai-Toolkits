...
- **输入图也计费（5.0 pro）**：控制台定价页（2026-09-21）写「输入图（首张免费）0.02 元/张 · 输出图
  0.3 元/张（输出图 ≤261 万像素）」，超过 261 万像素是 **0.6 元/张**（2026-09-23 用户查证）。换成
  档位就是 **1K 与 1.5K 同价 0.3、2K 0.6**：1.5K 各比例最大 241 万，2K 最小 421 万，分界线落在两档之间，
  所以按规格计费组写两行即可（`2K` 0.6 + 不写尺寸 0.3）；5.0 lite 是 0.22 元/张、不收输入费。`usage.input_images` 是
