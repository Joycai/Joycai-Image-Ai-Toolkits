...
| 9 | 按张计费的端点：输入图有没有单独计费？张数是**实际放进请求体**的（截断后、丢掉读不出的之后）还是附件数？一张没交付时收不收？ | 13 §7 | 126 |
| 10 | 上游报了钱（`cost_in_usd_ticks` 类）的面：读了吗？「没报」与「0」分得开吗？经中转时是否误读？流被放弃时报价还在吗？ | 13 §7、06 §1 | 127 |
