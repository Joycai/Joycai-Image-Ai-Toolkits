...
| 9 | 按张计费的端点：输入图有没有单独计费？张数是**实际放进请求体**的（截断后、丢掉读不出的之后）还是附件数？一张没交付时收不收？**视频面的首帧 / 参考图也算**（xAI 视频每张 +$0.01，标价页不写），按秒 / 按条计价的组有没有地方填这一侧？ | 13 §7、14 §2 | 126、129 |
| 10 | 上游报了钱（`cost_in_usd_ticks` 类）的面：读了吗？「没报」与「0」分得开吗？经中转时是否误读？流被放弃时报价还在吗？异步任务的报价**只在终态回包**：提交时记的行，终态有没有被报价改写？ | 13 §7、06 §1、14 §3.8 | 127、129 |
| 11 | 计费相关的请求默认值（质量、分辨率、`n`、`audio`）是否**明发**？不明发 = 用量记录缺规格 + 本地估价按错档 | 13 §4.2、§7；14 §3 | 123、125 |
