...
| 提示词助手 · 库编辑模式左栏 | `10h` 把参考图栏换成知识库文件树 | 仍是参考图栏 | 未做。树本身是个新组件，而且换掉左栏意味着库编辑模式下看不到参考图——这个取舍值得单独想，不该顺手做掉 |
| 提示词助手 · 写入权限卡 | `10h` 三个开关（允许写入 / 逐条确认 / 备份到 .bak） | 未做 | 三个新的持久化设置，且「关闭逐条确认」会绕过这一整套审批闸门。等它自己一轮 |
