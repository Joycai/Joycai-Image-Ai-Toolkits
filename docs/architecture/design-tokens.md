...
**预算**：每屏全宽玻璃 ≤ 1，同屏可见玻璃 ≤ 3，玻璃只贴滚动区的边。

预算原来的理由是"一次全宽模糊在集显 4K 上约 19ms / 帧"——**这个数在本项目现在的渲染器下测不出来**。`lib/bench/render_bench.dart` 的玻璃阶梯在开发机集显、4K 最大化下实测：0 / 1 / 2 / 3 / 4 层滤镜分别是 9.90 / 10.16 / 10.51 / 10.78 / 11.19 ms GPU 时间，即一层全宽 G1 模糊约 **0.32ms**，四层合计 1.3ms（整帧 11ms）。19ms 是 `windows/runner` 钉死 Skia 之前测的（Impeller 无法导入 video_player_win 的 DXGI 纹理），当成另一个渲染器的数字看，不是这个的。

`BackdropGroup` + `BackdropFilter.grouped` 把四层合成一份快照实测 11.20 对 11.19——没有收益，所以 `AppGlass` 不带 `grouped` 参数。要重开这个话题，先重跑阶梯。

预算照旧保留，但理由换成诚实的那几条：三层已是设计读得清的上限；每个滤镜仍然是一次全目标回读，代价随面积 × DPR² 走，而这里没人测过的硬件上它可能不是 0.32ms；有个上限，工作台才不会再飘回七层。预算**不是**这个 app 的帧时间瓶颈——窗口底才是，烘之前 7.3ms。
...
**这四层每帧只画一次贴图，不是四次满窗填充。** 配方是静态的（只随主题、强调色和窗口尺寸变），所以 `BakedAuroraBackdrop` 把它按 1/4 分辨率烘成一张 `ui.Image`，每帧交给 GPU 一个贴图四边形。这里以前是四层叠满窗的 `SizedBox.expand` 装饰套在 `RepaintBoundary` 里，注释写着"重绘边界里画一次"——那句是错的：重绘边界约束的是 Dart 侧哪段绘制代码重跑，不是 GPU 执行什么，而 raster cache 有尺寸上限，4K 满窗远在上限之外。开发机集显 4K 最大化实测 GPU 时间：空窗 2.58ms，四层实画 9.91ms，三层纯色填充（去掉渐变着色器）6.94ms，烘成贴图 4.84ms；整个 app 从 11.45 降到 6.89ms/帧。1/4 分辨率之所以允许，是因为配方里没有硬边——`test/baked_backdrop_test.dart` 逐像素守着这一点，谁往里加了边或图案它就红。
...
| 推理强度滑块的档位（D1c） | 六档等距恒定 | 档位按 wire 走（`LLMDispatcher.reasoningLadder`）：① 六档；④ adaptive 无「关闭」；只有开关的 wire 为「默认 / 开启」 | 发出去一样的两档是没有效果的旋钮；不支持时仍画满六档 |
| 推理强度不可用态（D1c） | 滑块置灰，拇指停在 Default | 置灰，拇指固定在「关闭」 | 用户决定：不支持推理的模型读作"关闭"。存储值不动，换回支持的类型时恢复 |
| 工作台参数格下拉高度（A1 / A2） | 画稿里参数格下拉为 30 | 32 | 规格文字写输入一律 32，图像面板已按 32 实现；视频面板跟随 |
| 单行输入 / 选择框高度 | 各稿按 32（手机表单 44） | 一律钉死：`pinnedFieldInset` + `constraints: tightFor(h)`；主题 `visualDensity` 固定 standard | InputDecorator 的描边包的是内容而不是给它的盒子；桌面默认 compact 密度再减 8px。浮动标签表单（首启向导、费用组编辑、图片尺寸、文件改名）未钉，钉高会裁掉标签 |
