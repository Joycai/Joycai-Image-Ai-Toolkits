...
| 工作台「添加文件夹」 | `10c` 画成主色 9% 底 + 主色 28% 描边（tonal） | `AppButton.secondary`（中性描边） | `7a` 的第 6 条说明要的是**降权**，让整屏只剩一个实心主题色按钮。中性描边降得更多，同时守住 §1「主题色只出现在选中态、主 CTA 与徽标上」。tonal 会把主色花在用户**不该**点的那个动作上，而这正是 `secondary` 当初从 tonal 改成描边的原因 |
