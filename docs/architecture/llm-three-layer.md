...
- **路由答案**：单发、提交即计费、不声明工具；超时 5 分钟起，按一次请求可能画的张数
  ...
  **`output_size` 只在没选档位时发布**（方舟回显的 `size`，且各张一致；2026-09-23）：pro 的 1K / 1.5K 同价
  0.3、2K 0.6，自然的计费组是「`2K` 0.6 + 不写尺寸 0.3」两行；「自动」画的是上游默认 2K，没有回显就落到
  0.3 行、少记一半。选了档位则不发：计价看档位，而像素按「表里列出的档位」就近归档，会把 1.5K 的
  2048×1152 归到这张两行表唯一的 `2K` 行。拆图层的底图与各层尺寸不同，不发。
- **5.0 pro 的分辨率有「自动」（`not_set`，不发 `size`，2026-09-23）。** 这是拆图层够到上游默认
  ...
  按档位写的费用组能匹配；「自动」靠回显的 `output_size` 匹配（上一条）。
