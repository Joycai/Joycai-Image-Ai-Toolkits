...
| 拆图层「自动」的计费 | 2026-09-23 给 5.0 pro 加了分辨率「自动」（拆图层够到上游 `auto`）。生成模式的「自动」已靠回显的 `output_size` 按 2K 计价；拆图层的底图与各层尺寸各不相同，不发布尺寸，整组落到不写尺寸的 0.3 行。未知的是方舟怎么给 `auto` 出的图计价：原图若在 241–421 万像素之间（1.5K 与 2K 之外），是按 261 万分界线、还是按某个档位收，要对照账单；各层是否各按自己的像素计价也未知 |
