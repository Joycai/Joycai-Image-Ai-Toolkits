...
| 视频的输入图 | 首帧 / 参考图是否单独计费，四家文档都没写；没有视频协议发布张数，且只有单位为「张」的组收输入费（`PricingGroup.chargesInputImages`）。要做得两处一起开 |
...
| xAI 的 `cost_in_usd_ticks` | xAI 在 `usage` 里回报这次请求的实扣金额（1 tick = $10⁻¹⁰，2026-09-21 实测，`api/usage.md` §5）。它比任何费率表都准，但接进来要在用量行上加一种「上游报价」的计费口径，这一轮没做——现在仍按计费组的费率表算 |
| ~~xAI 的质量参数~~ | 已做（`2026-09-xai-quality.md`，v4.26.0）：2.0 发 `quality`（low / medium，默认 medium）与 `1.5k`；初代分到自己的表。剩一条：`auto` 由模型定档、不给；**250 最小面板宽下英文 Medium 仍会省略**（半栏 43px 对 46px），gpt-image-2 的四档轨道在同一宽度下同样——只在用户把栏拖到极限时出现，不做 |
...
| 输入图 2 | 一次请求出多张（xAI `n>1`）时输入图收一次还是乘张数 | 发一次 `n:2` 带参考图的请求，读回包的 `cost_in_usd_ticks`（不必等账单）。`n:1` 已实测：每张输入图 $0.01、线性、无免费 | **不响**：应用按每次请求收一次（应用自己只发 `n:1`，所以只影响别处复用这张费率表的人） |
