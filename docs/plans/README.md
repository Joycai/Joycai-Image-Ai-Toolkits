...
- **保存之后**：新建组保存后追加到列表末尾并保持选中（稿 1f）；编辑已有组保存后同样保持选中，稿没说，取一致。
