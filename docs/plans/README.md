...
| xAI 的质量参数 | 标价按分辨率 × 质量（Low / Medium）分档，应用不发质量参数，实测默认按 **Medium** 计（1K $0.06）。所以 xAI 的按规格计费组现在只能按尺寸写档、填 Medium 的价；要省钱或要档位表按质量命中，得先在能力表里给 `grok-imagine-image-2.0` 加质量控件并确认参数名 |
