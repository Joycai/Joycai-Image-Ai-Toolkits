...
- **review 第 2 轮**（opus，修复本身 + 全范围复扫）：2 条 MINOR，都是文档——① 轮询侧的对称不变量（穿透式轮询返回前剔保留键）
  只活在注释与测试里 → 写进 `llm-three-layer.md` 同一条；② 坑 126 的对策只覆盖「铺 usage」一种形状，Veo 是第九处且形状不同
  （整个 body 当信封穿透）→ skill 源与快照同改。代码无新发现：pending 的 map 到不了两个读者；其余四面与两个 sim 分支都是自造信封。
