...
| 片 | 内容 | 涉及文件 | 验收 | 状态 |
|---|---|---|---|---|
| 1 | 数据与解析：v44 `llm_models.max_output_tokens`（onCreate + onUpgrade）、`LLMModel.maxOutputTokens`、`LLMModelConfig.maxOutputTokens`、resolver 透传、备份导入不剥该列 | `database_migrations.dart` · `database_service.dart`（dbVersion 44）· `models/llm_model.dart` · `llm_types.dart` · `llm_config_resolver.dart` | 现有 DB 测试绿；新建库与升级库都有该列 | ☐ |
