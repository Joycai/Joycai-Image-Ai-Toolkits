...
- **Review 第三轮（opus）** 无 BLOCKER / MAJOR；修了：① 第二轮给 `AppDropdown` 的 `trailing` 限宽 200 仍是非弹性子项，
  先于标签布局——390pt 手机页上组名只剩 74px，320pt 上剩 4px，再窄仍溢出。改为 `Flexible`（标签 3 : 摘要 2，
  摘要再封顶 200），测试改用真实调用形状（large + 前缀图标，366 / 296 / 240 三档）并断言标签宽于摘要；
  ② 英文「1 images · 1 free」——输入张数用自己的带复数的字符串 `usageInputSentCount`；③ Midjourney 那段缩进；
  ④ 架构注记里「文生图不发布这个键」补上「除非上游回报了 0」。
- **Review 第四轮（opus）** 无 BLOCKER / MAJOR；第三轮的 3 : 2 弹性分配矫枉过正——`Expanded` 是紧约束、`Flexible` 是松约束，
  两头换不了地方：组名短时摘要照样被截（「按 token · 0.30 / 0.03 / 2.50」在 422 栏里丢了输出价），摘要短时右边空出一截。
